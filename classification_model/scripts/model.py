...
class BiModel(nn.Module):
    ...
    def __init__(self, D_m, D_g, D_p, D_e, D_h,
        ...
        # self.text_linear = nn.Linear(100,100)
        # self.audio_linear = nn.Linear(2000,712)
        # self.video_linear = nn.Linear(512,100)
        self.D_g = D_g
        self.D_p = D_p
        self.D_e = D_e
        self.D_h = D_h
        # self.audio_linear = nn.Linear(2000,2000)
        ...
        self.matchatt = MatchingAttention(2 * D_e, 2 * D_e, att_type='general')
    ...
    def forward(self, acouf, textf, visuf, qmask, umask, att2=True):

        # acouf_h = F.relu(self.audio_linear(acouf))
        # textf_h = F.relu(self.text_linear(textf))
        # visuf_h = F.relu(self.video_linear(visuf))
        # U = torch.cat((acouf, textf, visuf),dim=-1) # Uncomment to multi modal evaluation
        U = acouf
        ...
        if att2:
            return log_prob, alpha, alpha_f, alpha_b, log_prob
        else:
            return log_prob, [], alpha_f, alpha_b, log_prob
